...
function setup(runtime2) {
  ...
  if (!network248) {
    runtime2.log(`Unknown chain name ${evmConfig.chainSelectorName}`);
...
function reportDataSetup(typing, data) {
  if (!typing)
    throw new Error("please specify the data type");
  const reportData = encodeAbiParameters(parseAbiParameters(typing), data);
  return reportData;
}
function reportSign(runtime2, typing, data) {
...
function readOnchain(runtime2, evmClient, contractAbi, funcName, contractAddr, args) {
  ...
  const msgCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: contractAddr,
  ...
  const callResult = decodeFunctionResult({
    abi: contractAbi,
    functionName: funcName,
    data: bytesToHex2(msgCall.data)
  });
  runtime2.log(`call result ${callResult}`);
  return callResult;
}
...
function fetchTokensPrices(runtime2, evmClient, tokens) {
  ...
  for (const token of tokens) {
    if (!supportedTokensAddresses.includes(token.address)) {
      runtime2.log(`unsupported tokens ${token.address}`);
      continue;
    }
    ...
    const actualPrice = Number(tokenPriceResult);
  ...
  runtime2.log(`token prices in USD ${tokenPricesInUsd}`);
  return tokenPricesInUsd;
}
var calculateShare = (runtime2, tokens, evmClient) => {
  const tokensValue = fetchTokensPrices(runtime2, evmClient, supportedTokensPriceFeeds);
  let totalCollateralValueInUsd = 0n;
  tokensValue.forEach((price) => {
    totalCollateralValueInUsd += price[1];
    runtime2.log(`[CALCULATING-SHARE]: token ${price[0]} is ${price[1]}`);
  });
  runtime2.log(`[CALCULATING-SHARE]: total values in USD ${totalCollateralValueInUsd / 10n ** 8n}`);
  return totalCollateralValueInUsd / 10n ** 8n;
};
function handleDeployCollaterals(runtime2, collaterals, user, name, symbol2) {
  ...
  try {
    ...
    runtime2.log("writing a tokenizer creation report");
    const depositColResult = reportWrite(runtime2, signedReport, evmConfig, evmClient, evmConfig.vaultAddress);
    const txHash = bytesToHex(depositColResult?.txHash || new Uint8Array(32));
    runtime2.log(`tokenizer creation's hash ${txHash}`);
    return txHash;
  } catch (error) {
    runtime2.log(`[Deploy-Tokenizer]: something went wrong on depositing collateral ${error}`);
    return "";
  }
}
function handleDepositCollaterals(runtime2, collaterals, user) {
  const { network: network248, evmConfig } = setup(runtime2);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  ...
  try {
    ...
    const signedReport = reportSign(runtime2, "uint8 actionCode, uint256 vaultId, address user, address[] collaterals, uint256[] amounts, uint256 sharesToMint", [
      ...
      filteredCollaterals,
      collateralsAmounts,
      sharesToMint * 10n ** 18n
    ]);
    runtime2.log("writing a collateral deposit report");
    const depositColResult = reportWrite(runtime2, signedReport, evmConfig, evmClient, evmConfig.vaultAddress);
    const txHash = bytesToHex(depositColResult?.txHash || new Uint8Array(32));
    runtime2.log(`collateral deposit's hash ${txHash}`);
    return txHash;
  } catch (error) {
    runtime2.log(`[Deposit-Collateral]: something went wrong on depositing collateral ${error}`);
    return "";
  }
}
function handleRedeemCollaterals(runtime2, collaterals, user) {
  const { network: network248, evmConfig } = setup(runtime2);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const sharesToBurn = calculateShare(runtime2, collaterals, evmClient);
  try {
    runtime2.log("signin a collateral redemption report");
    const ACTION_REDEEM_SHARES_ERC20 = 2;
    const signedReport = reportSign(runtime2, "uint8 actionCode, uint256 vaultId, address user, uint256 sharesBurned, address receiver", [
    ...
    const redeemResult = reportWrite(runtime2, signedReport, evmConfig, evmClient, evmConfig.vaultAddress);
    const txHash = bytesToHex(redeemResult?.txHash || new Uint8Array(32));
    runtime2.log(`collateral redemption's hash ${txHash}`);
    return txHash;
  } catch (error) {
    runtime2.log(`[Redeem-Collateral]: something went wrong on redeem collateral ${error}`);
    return "";
  }
}
...
var onDeployTokenizer = (runtime2) => {
  ...
  try {
    ...
    runtime2.log(`Tokenizer Deploy FAILED: ${error}`);
    return "";
  }
};
var onDepositCollaterals = (runtime2) => {
  const { network: network248 } = setup(runtime2);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  try {
    const depositCollateralsHash = handleDepositCollaterals(runtime2, supportedTokensPriceFeeds, USER);
...
var onRedeemCollaterals = (runtime2) => {
  try {
    const redeemTxHash = handleRedeemCollaterals(runtime2, supportedTokensPriceFeeds, USER);
    runtime2.log(`COLLATERALS REDEEMED: ${redeemTxHash}`);
    return redeemTxHash;
  } catch (error) {
    runtime2.log(`COLLATERALS REDEMPTION FAILED: ${error}`);
    return "";
  }
};
...
var initWorkflow = (config) => {
  ...
  return [
    handler(cron.trigger({ schedule: config.schedule }), onDeployTokenizer),
